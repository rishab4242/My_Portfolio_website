...
const ExperienceSection = () => {
  ...
  useEffect(() => {
    // Determine if the viewport is mobile (less than 768px)
    const isMobile = window.innerWidth < 768;

    // Configure IntersectionObserver with different settings for mobile
    const observerOptions = {
      threshold: isMobile ? 0.4 : 0.2, // Higher threshold for mobile to delay trigger
      rootMargin: isMobile ? "100px" : "50px", // Larger rootMargin for mobile
    };
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <div
          className={`bg-gray-900 rounded-2xl shadow-2xl transition-all duration-1000 transform overflow-hidden border border-gray-700 hover:shadow-blue-500/30 hover:shadow-2xl hover:-translate-y-2 hover:border-blue-500/50 hover:scale-[1.02] ${
        ...
        >
          ...
          <div className="p-8">
            ...
            <div className="mb-8">
              <h4
                className={`text-lg font-semibold text-gray-200 mb-4 transform transition-all duration-700 delay-500 ${
                  isVisible
                    ? "translate-x-0 opacity-100"
                    : "translate-x-4 opacity-0"
                }`}
              ...
              <div className="flex flex-wrap gap-3">
                {skills.map((skill, index) => (
                  <div
                    key={index}
                    className={`flex items-center gap-2 bg-gradient-to-r from-gray-800 to-gray-700 px-4 py-2 rounded-full border border-gray-600 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/30 transition-all duration-300 transform hover:scale-110 hover:-translate-y-1 cursor-pointer ${
                      skillsVisible
